...
def generateHomography(src_img, dst_img, ransacRep=5.0, option="SURF",ratio=0.75):
	...
	src_kp,src_features=findAndDescribeFeatures(src_img,opt=option)
	dst_kp,dst_features=findAndDescribeFeatures(dst_img,opt=option)

	good=matchFeatures(src_features,dst_features,ratio=ratio)
